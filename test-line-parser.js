...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.includes('☆') && currentDay > 0) {
    const courseName = line.replace('☆', '').trim();
    console.log(`行${i}: 找到课程 "${courseName}"`);
    ...
    if (nextLine && nextLine.startsWith('(')) {
      ...
      if (detailMatch) {
        console.log(`  ✓ 解析成功：${courseName} | 周${currentDay} | 节${detailMatch[1]}-${detailMatch[2]} | ${detailMatch[4]} | ${detailMatch[5]}`);
        courses.push({
          courseName,
