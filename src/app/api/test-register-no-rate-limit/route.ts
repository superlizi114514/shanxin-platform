...
/**
 * 测试注册 API - 移除 rate limiting 测试
 */
export async function POST(request: NextRequest) {
  const logs: string[] = [];

  try {
    // Step 1: 解析 JSON (without rate limiting)
    ...
    logs.push(`validated: email=${email}, isTeacher=${isTeacher}, phone=${phone}`);
    ...
    return NextResponse.json({
      ...
      userId: user.id,
      logs,
      message: "Registration successful (test without rate limiting)",
    });
