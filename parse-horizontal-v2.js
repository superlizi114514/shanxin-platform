...
async function parseHorizontalTableV2() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 检测课程名行（只在未处理过时处理）
    if (line.includes('☆') && currentPeriod && !processedForPeriod && !line.trim().startsWith('(')) {
      const names = line.split('☆').filter(p => p.trim() && p.length >= 2 && p.length <= 40);

      // 验证课程名：不应该包含 (或) 或/
      const validNames = names.filter(n => !n.includes('(') && !n.includes(')') && !n.includes('/'));
      ...
      if (positions.length === pendingCourseNames.length) {
        ...
        for (let colIdx = 0; colIdx < positions.length; colIdx++) {
          ...
          if (match) {
            ...
            // 尝试更宽松的正则
            const looseMatch = colText.match(/\((\d+)-(\d+) 节\)/);
            if (looseMatch) {
              console.log(`  ~ ${pendingCourseNames[colIdx]}: 找到节次但详情不完整`);
            } else {
...
parseHorizontalTableV2().catch(console.error);
