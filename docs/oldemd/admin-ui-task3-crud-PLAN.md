...
**完成状态**: 用户管理、商家管理、新闻管理已完成 ✅，信息大全模块可选做。

---

## 实现 Phase

### Phase 1: 用户管理模块（复杂度：高） ✅ 已完成
...
- [x] 基本信息卡片（头像、昵称、邮箱、手机号、角色）
- [x] 学生信息卡片（学号、校区、系、班级）
- [x] 统计数据卡片（商品数、订单数、收藏数）
- [x] 操作按钮（编辑、重置密码、禁用/启用）

#### 1.3 编辑页 UI
**文件**: `src/app/admin/users/[id]/edit/page.tsx`

- [x] 表单字段：昵称、手机号、角色选择、状态切换
- [x] 表单验证使用 Zod
- [x] 保存/取消按钮
- [x] 返回导航
...
**文件**: `src/app/api/admin/merchants/[id]/stats/route.ts`
- [x] **GET** - 获取商家统计数据

#### 2.2 详情页 UI
**文件**: `src/app/admin/merchants/[id]/page.tsx`

- [x] 商家信息卡片（logo、名称、描述、地址）
- [x] 评分数据展示（平均分、评价数）
- [x] 图片画廊（环境图展示）
- [x] 认领状态标识
- [x] 操作按钮（编辑、认证、上下架）
...
- [x] Markdown 编辑器（使用 textarea 支持 Markdown）
- [x] 图片上传（封面图）
- [x] 分类选择（下拉框）
- [x] 发布状态切换
- [x] Zod 验证
...
### Phase 4: 信息大全模块（复杂度：低） ⏭️ 可选/跳过

信息大全模块的详情/编辑功能可根据需要后续添加。当前列表页已支持分类管理和文章管理，满足基本使用需求。
...
**完成模块**: 用户管理、商家管理、新闻管理 ✅
**新增页面**: 6 个（用户详情/编辑、商家详情/编辑、新闻详情/编辑）
**新增 API**: 4 个端点（用户管理 API、商家统计 API）
**修改页面**: 2 个（商家列表、新闻列表添加详情按钮）
