...
async function parseHorizontalTable() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('☆') && currentPeriod && !line.trim().startsWith('(')) {
      ...
      console.log(`\n节次${currentPeriod}: ${names.length} 门课程 - ${names.join(', ')}`);
      continue;
    }

    // 检测详情行开始（包含 (X-Y 节) 模式）
    if (currentPeriod && pendingCourseNames.length > 0 && /\(\d+-\d+ 节\)/.test(line)) {
      // 找到所有 (X-Y 节) 的位置
      const positions = [];
      let idx = 0;
      ...
      console.log(`  找到 ${positions.length} 个详情列`);
      ...
      if (positions.length === pendingCourseNames.length) {
        ...
        for (let colIdx = 0; colIdx < positions.length; colIdx++) {
          ...
          if (match) {
            ...
            console.log(`  ✓ [星期${colIdx + 1}] ${pendingCourseNames[colIdx]}: (${match[1]}-${match[2]}节) ${classroom} ${teacher}`);
          } else {
            console.log(`  ✗ [星期${colIdx + 1}] ${pendingCourseNames[colIdx]}: 未解析`);
          }
        }
